...
import { useGetConnectedAgentMessageThreads, useGetConnectedUserAgents } from "hooks/useGetConnectedAgents";
...
export default function ConversationTab() {
    ...
    const renderMessageContent = (message: Message) => (
        <div className={`rounded-xl text-white p-4 py-3 shadow-sm font-satoshi ${message?.sender === user?.id
            ? ' bg-black text-white '
            : 'bg-white text-black'
            }
      `}>
                                    ...
                                    <div className='sticky top-0 z-10 py-1 flex  items-center justify-between border-b  bg-white px-4  font-bold'>
                                        <div className="flex gap-2">
                                            <CustomAvatar
                                                ...
                                                size={'2.8rem'}
                                            >
                                                {getInitials(activeAgent?.firstName, activeAgent?.lastName) || 'SH'}
                                            </CustomAvatar>
                                            <div className="flex flex-col gap-0">
                                                <span className="text-sm ">{activeAgent?.firstName} {activeAgent?.lastName}</span>
                                                <span className="flex text-xs items-center font-normal -mt-1">Online<Dot color="green" /></span>
                                            </div>
                                        </div>
                                          ...
                                          <Image
                                           ...
                                           />
                                            <div className="flex flex-col gap-0">
                                                ...
                                                <span className="flex text-xs items-center font-normal -mt-1">California</span>
                                            </div>
                                    ...
                                    <div className='h-full pb-10 overflow-scroll transition-all duration-100 scrollbar-thumb-[#E8804C]'>
                                        {messages
                                            ...
                                            .map((message: Message) => (
                                                <div
                                                    key={message._id}
                                                    className={`font-satoshi flex flex-col px-8 pt-6 ${message?.sender === user?.id
                                                        ? 'items-end  '
                                                        : 'items-start'
                                                        }`}
                                                >
