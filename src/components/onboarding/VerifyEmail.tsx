  ...
  const onSubmit = (data:IVerifyEmailInput) => {
    mutateVerifyAgentCode({
      ...
      onSuccess: (data) => {
        ...
        console.log(data , data?.data.verifyOtp?.access_token)
    
          const newAgent = { is_authenticated: true }

          const secureLogin = generateSHAString(AgentEmail ?? 'secure@ocreal_agent');

          console.log(secureLogin)

          storeCookie({ key: SECURE_LOGIN_KEY, value: secureLogin });

          storeCookie({ key: AUTH_TOKEN, value: data?.data?.verifyOtp?.access_token });
